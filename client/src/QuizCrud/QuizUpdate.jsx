...
const QuizUpdate = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const res = await axios.put(
                `http://localhost:8000/api/v1/admin/updatequiz/${quizId}`,
                data,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
            ...
            if (backendErrors) {
                Object.keys(backendErrors).forEach((field) => {
                    setFormError(field, { type: "server", message: backendErrors[field] });
                });
            } else if (globalErrors) {
                setFormError("root", { type: "server", message: globalErrors });
            }
        } finally {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <h2 className="text-3xl font-bold text-gray-800 mb-6">✏️ Edit Quiz</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                >
                    ...
                    <span>{isSubmitting ? 'Updating Quiz...' : 'Update Quiz'}</span>
                </button>
            </form>
        </div>
    );
